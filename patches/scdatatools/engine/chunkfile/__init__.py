...
class ChunkFile:
    def __init__(self, chunk_file: typing.Union[str, Path, P4KInfo, P4KExtFile], *args, **kwargs):
        ...
        self.header = chunk_file_header_for_signature(self.raw_data[:4]).from_buffer(self.raw_data, 0)
        self.chunks = {}
...
class GeometryChunkFile(ChunkFile):
    def _find_mtl(self, path):
        ...
        if isinstance(self._data, P4KFile):
            try:
                path = path.lower()
                if f := self._data.NameToInfoLower.get(path if path.startswith("data") else f"data/{path}"):
                    return f
                elif f := self._data.getinfo((Path(self.filename).parent / path)):
                    return f
                elif "female_v2" in self.filename and (
                    f := self._data.getinfo(Path(self.filename.replace("female_v2", "male_v7")).parent / path)
                ):
                    # see note below...
                    return f
            except KeyError:
            ...
            if (f := self._data / path).is_file():
                ...
                return f
            elif "female_v2" in f.parts and (f := Path(f.as_posix().replace("female_v2", "male_v7"))).is_file():
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for chunk in self.chunks.values():
            ...
            elif chunk.chunk_type == ChunkType.Character_Skeleton and chunk.chunk_header.version in [0x901]:
                self._901 = True
                bones_forest_id = {}
                subobj_mapping = {}
                for i in range(len(chunk.joints)):
                    bones_forest_id.setdefault(chunk.joints[i].parent_id, []).append(i)
                    if chunk.joints[i].subobj_id not in subobj_mapping:
                        subobj_mapping[chunk.joints[i].subobj_id] = i
                    chunk.joints[i].bone_id = i

                def recurse_resolve(node_id: int, parent_id: int):
                    node = chunk.joints[node_id]
                    if node.controller_name not in self.nodes:
                        self.nodes[node.controller_name] = node
                    if parent_id != -1:
                        node.parent = chunk.joints[parent_id]
                        chunk.joints[parent_id].children.append(node)
                    else:
                        self._root_node_names.append(node.controller_name)
                    if node_id in bones_forest_id:
                        for x in bones_forest_id[node_id]:
                            recurse_resolve(x, node_id)
                    return

                root_bone = bones_forest_id[-1][0]
                recurse_resolve(root_bone, -1)
    ...
    def load_mesh_file(self):
        ...
        if isinstance(self._data, P4KFile):
            try:
                ...
                logger.error(f'Could not load the mesh component for "{self.filename}": "{self.mesh_file}"')
                self.mesh_component = None
        ...
        if self.mesh_component is not None:
            for chunk in self.mesh_component.chunks.values():
                ...
                if isinstance(chunk, Node):
                    ...
                    self.nodes[chunk.name] = chunk
                    if chunk.parent_id >= 0 and chunk.name not in (parent := self.chunks[chunk.parent_id]).children:
    ...
    def lods(self):
        ...
        if self._p4kinfo is not None:
            ...
            lods = {lod.stem.split("_lod")[-1].split(".")[0]: lod for lod in model_path.parent.glob(lod_search)}
