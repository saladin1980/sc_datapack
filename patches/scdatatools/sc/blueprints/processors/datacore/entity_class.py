...
def process_geometry_resource_params(bp: "Blueprint", component, record: "Record", tags="", *args, **kwargs) -> bool:
    if component.name == "SGeometryDataParams":
        ...
        if geom_path:
            ...
            try:
                ...
                bp.log(f'could not dump tint {tint_id} for "{geom_path}"')
    ...
    if "SubGeometry" in component.properties:
        for sg in component.properties.get("SubGeometry", []):
            process_geometry_resource_params(bp, sg, record=record, tags=component.properties.get("Tags", ""))
...
def load_item_port(bp: "Blueprint", item_port_name: str, record: Record, parent_geometry=None, parent_loadout=None):
    if parent_geometry is None:
        if not bp.entity:
            raise ValueError("Must specify parent_geometry or have a bp.entity set")
    ...
    try:
        for entry in component.properties["loadout"].properties.get("entries", []):
            try:
                ...
                item_port = load_item_port(
                    bp=bp,
                    item_port_name=port_name,
                    record=item_port_entity,
                    parent_geometry=parent_geom,
                    parent_loadout=parent_loadout,
                )
    ...
    for parent_part_name, params in parent_parts.items():
        ...
        for child_part_name in params["children"]:
            if child_part_name not in parts:
                bp.log(f"did not find child part {child_part_name} for {parent_part_name} in " f'{parent_geom["name"]}')
                continue
